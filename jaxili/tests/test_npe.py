...
def test_init():
    ...
    ), "The model hyperparameters are not None."
...
def test_append_simulations():
    ...
    assert inference._dim_cond == x_train.shape[1], "The number of the conditionning variable is wrong."
...
def test_build_neural_network():
    ...
    assert inference._model is not None, "The model is None."
    ...
    params = inference._model.init(jax.random.PRNGKey(0), theta_train, x_train)

    test_theta = inference._model.apply(params, inference._train_dataset.theta, method="standardize")

    npt.assert_allclose(standardized_theta, test_theta, rtol=1e-5, atol=1e-5)

    test_embedding = inference._model.apply(params, inference._train_dataset.x, method="embedding")
    ...
    log_prob = inference._model.apply(params, inference._train_dataset.theta, inference._train_dataset.x, method="log_prob")
    assert log_prob.shape[0] == len(inference._train_dataset), "The shape of the output of log_prob method is wrong."

    samples = inference._model.apply(params, inference._train_dataset.x[0], num_samples=10_000, key=jax.random.PRNGKey(0), method="sample")
